...
func (b *BoltAdapter) Connect(ctx context.Context, uri, user, pass string) error {
	opts := neo4j.Config{}
	drv, err := neo4j.NewDriverWithContext(uri, neo4j.BasicAuth(user, pass, ""), func(c *neo4j.Config) { *c = opts })
	if err != nil {
...
func (b *BoltAdapter) IngestBatch(ctx context.Context, nodes []Node, rels []Relationship) error {
	...
	if len(nodes) > 0 {
		_, err := sess.ExecuteWrite(ctx, func(tx neo4j.ManagedTransaction) (any, error) {
			...
			_, err := tx.Run(ctx, cypher, map[string]any{"batch": batch})
			return nil, err
		})
		if err != nil {
			return err
	...
	if len(rels) > 0 {
		_, err := sess.ExecuteWrite(ctx, func(tx neo4j.ManagedTransaction) (any, error) {
			// For relationships, match existing nodes and MERGE the relationship
